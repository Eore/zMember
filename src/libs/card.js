...
module.exports = (data, pipe) => {
  ...
  PDF.image("./bitmap.png", 0, 0, { width: conv(lebar), height: conv(tinggi) })
    .fontSize(10)
    .text(data.id, 10, conv(tinggi) - 35, {
      width: conv(lebar),
      height: conv(tinggi)
    })
    .fontSize(size())
    ...
    .moveTo(0, 0)
    .translate(
      PDF.page.width / 2.5 - GenQR.size - 2,
      PDF.page.height / 2.5 - GenQR.size - 2
    )
    .path(GenQR.path)
    .fill("black", "even-odd");
